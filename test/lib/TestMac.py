...
def CheckFileType(test, file, archs):
  ...
  o = proc.communicate()[0].decode('utf8').strip()
...
class XcodeInfo(object):
  ...
  def _XcodeVersion(self):
    o = subprocess.check_output(['xcodebuild', '-version']).decode('utf8')
    lines = o.splitlines()
    version = lines[0].split()[-1].split('.')
    version = ''.join('%02d' % int(x) for x in version)
  ...
  def SDKBuild(self):
    if 'SDKBuild' not in self._cache:
      ...
      self._cache['SDKBuild'] = self._cache['SDKBuild'].decode('utf8')
