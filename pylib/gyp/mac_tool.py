...
class MacTool(object):
  ...
  def ExecCopyInfoPlist(self, source, dest, convert_to_binary, *keys):
    ...
    lines_bytes = plistlib.dumps(plist, fmt=plistlib.FMT_XML)
  ...
  def _WritePkgInfo(self, info_plist_path):
    ...
    with open(info_plist_path, 'rb') as info_plist_file:
      plist = plistlib.load(info_plist_file)
    if not plist:
      return
  ...
  def ExecMergeInfoPlist(self, output, *inputs):
    ...
    try:
      with open(output, 'wb') as f:
        plistlib.dump(merged_plist, f, fmt=plistlib.FMT_XML)
    except TypeError:
      plistlib.dump(merged_plist, output, fmt=plistlib.FMT_XML)
  ...
  def _LoadProvisioningProfile(self, profile_path):
    ...
    with tempfile.NamedTemporaryFile(delete_on_close=False) as temp:
      ...
      with open(temp.name, 'rb') as reopened_temp:
        return plistlib.load(reopened_temp)
  ...
  def _GetCFBundleIdentifier(self):
    ...
    with open(info_plist_path, 'rb') as info_plist_file:
      info_plist_data = plistlib.load(info_plist_file)
  ...
  def _InstallEntitlements(self, entitlements, substitutions, overrides):
    ...
    with open(target_path, 'wb') as out_file:
      plistlib.dump(data, out_file, fmt=plistlib.FMT_XML)
    return target_path
